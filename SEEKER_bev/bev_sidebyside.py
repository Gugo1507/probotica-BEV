...
class DualRealTimeBEV:
    def __init__(self, width=640, height=480, fps=30):
        ...
        # =========SERIAL NUMBERS =========
        self.cam1_serial = "151422253555"
        self.cam2_serial = "151422250222"
        # ===============================================
        ...
        # ★ RESIZE SCALE (0.9 = 90% of original) ★
        self.scale = 0.9
        ...
        # ★ GAP BETWEEN BEVs (horizontal gap in pixels) ★
        self.gap_px = 20

        # Final canvas resolution (side-by-side)
        self.canvas_height = self.bev_height
        self.canvas_width = self.bev_width * 2 + self.gap_px
    ...
    def run(self):
        print("Running BEV side-by-side. Press 'q' to quit.")
        ...
        try:
            while True:
                ...
                # Left BEV
                canvas[:, 0:self.bev_width] = bev1

                # Right BEV starting position
                start_x = self.bev_width + self.gap_px
                canvas[:, start_x:start_x + self.bev_width] = bev2

                # ========== SHOW RESULT ==========
                cv2.imshow("Camera 1 RAW", img1)
                cv2.imshow("Camera 2 RAW", img2)
                cv2.imshow("BEV Combined Side-by-Side", canvas)
